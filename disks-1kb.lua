...
local test_d = require("suite-disk-tests-1kb")
...
function test_disk_warm(thread, client)
...
-- for disk tests the warmup phase can potentially take long, 
-- so we only run it once to emulate certain load patterns and
-- to allow repeatable data loading in general.
function test_disk_run_test(o, test)
    ...
    local stats_arg = { stats = { "cmd_get", "cmd_set"}, track = { "evictions", "extstore_io_queue" } } 
...
function test_disk(o)
    if o.extset then
        test_d.testset = o.extset
    end
    ...
    for _, tconfig in ipairs(test_d.testset) do
        ...
        plog("START", tconfig)
	if warm then
            test_disk_warm(o.warmthr, t.w)
	    warm = false
	end
        test_disk_run_test(o, t)
        plog("END")
    end
end
